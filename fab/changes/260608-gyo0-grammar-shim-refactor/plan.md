...
`hop --all <action>` and `hop <group> <action>` SHALL run the action across all matched repos. No-target batch becomes `hop --all pull` (replacing `hop pull --all`). A plural selection SHALL refuse non-batch, interactive actions: only the batch verbs (`pull`/`push`/`sync`) are permitted on a plural selection. Any other action — including `where` (it resolves a single path, meaningless across N repos), `cd`, `open`, or a PATH-binary/alias/function action — on a plural selection SHALL be refused with a usage error (exit 2).
...
3. **Plural-interactive guard**: a plural selection accepts only `pull`/`push`/`sync`; everything else (including `where`, which resolves a single path) errors with exit 2. — *Why*: cd/open/interactive-tool across N repos is nonsensical; a small allow-list is simpler and safer than an interactive-detection heuristic. — *Rejected*: trying to detect "interactive" PATH binaries dynamically (unreliable, over-engineered).
...
| 2 | Confident | Plural-selection guard = allow-list: only `pull`/`push`/`sync` permitted on `--all`/group; everything else (cd/open/`where`/tool) errors exit 2. | Intake Open Question. A small allow-list of meaningful-on-N-repos actions is simpler and safer than dynamic interactive-detection; cd/open/interactive-tool (and single-path `where`) across N repos is nonsensical. | S:80 R:65 A:85 D:75 |
