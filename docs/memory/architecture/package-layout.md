---
description: "`src/cmd/hop/` + `src/internal/<pkg>/`, cobra wiring, pre-cobra `--shim-plan` classifier (`shim_plan.go`), batch-verb action tokens (`batch_verb.go`), `help_dump.go` cobra-tree producer, conventions"
type: memory
---
# Package Layout
...
│   ├── clone.go, ls.go
│   ├── pull.go                   # pullSingle/pullBatch/pullOne + lastNonEmptyLine (runner logic only; no cobra factory — gyo0)
│   ├── push.go                   # pushSingle/pushBatch/pushOne (runner logic only; no cobra factory — gyo0)
│   ├── sync.go                   # syncSingle/syncBatch/syncOne + mentionsConflict + defaultSyncCommitMessage (runner logic only; no cobra factory — gyo0)
...
│   ├── tty.go                    # isTTY seam (term.IsTerminal on stdin), errNoTTY sentinel, noTTYHint stderr line (1x1u)
│   ├── config.go                 # config parent + nested init/where/scan/print factories; also wires the HIDDEN config add/config rm aliases
│   ├── config_scan.go            # shared plan-building helpers for `hop add`: validateConfigDir, buildScanPlan, slugify, conflict resolution, scanPlanSummary (no `config scan` cobra command — w2bj)
...
│   ├── skill.go                  # visible `hop skill` — newSkillCmd writes the embedded skill.md agent bundle verbatim to stdout (armh)
...
| TTY detection | `golang.org/x/term` v0.27.0 (+ indirect `golang.org/x/sys` v0.28.0) (1x1u); pins mirror the sibling `idea`. The `go` directive stays at `1.22` deliberately — term v0.27.0 builds clean against it. |
...
Each subcommand is exposed via a `func newXxxCmd() *cobra.Command` factory in its own file. `root.go::newRootCmd()` constructs the root and calls `AddCommand(newAddCmd(), newRmCmd(), newCloneCmd(), newLsCmd(), newShellInitCmd(), newConfigCmd(), newUpdateCmd(), newSkillCmd(), newHelpDumpCmd())`. That is **eight user-facing top-level subcommands** (`add`, `rm`, `clone`, `ls`, `shell-init`, `config`, `update`, `skill`) plus the hidden `help-dump`. **`pull`/`push`/`sync` are action tokens, not subcommands** (gyo0) — dispatched by `runRoot` → `runBatchVerb` (see [batch_verb.go](#batch_verbgo--batch-verb-action-tokens) below). The registry-edit verbs `add` / `rm` are the canonical top-level commands; `config add` / `config rm` survive as hidden aliases sharing the same `runAdd` / `runRm` bodies (wired under `config` in `config.go`, not here) — see [cli/subcommands § Migration: hidden config aliases](/cli/subcommands.md#migration-hidden-config-aliases). `main.go::main()`:
...
Owns the directory walk and repo classification for `hop add` (single-dir via `ClassifyOne`, recursive via `Walk` under `-r`). UI-free: knows how to recognize git working trees (vs worktrees, submodules, bare repos, no-remote repos) and how to follow symlinks safely with `(dev, inode)` loop dedup; does NOT know about groups, slugify, conflict resolution, YAML rendering, or stderr UX (those live in the CLI / yamled layers).
...
`ClassifyOne` (n1me; consumed by `cmd/hop/config_add.go`'s shared `runAdd` body) is the **single-dir entry point** for `hop add` (and its hidden `hop config add` alias): it classifies one already-canonical directory (no recursion, `opts.Depth` ignored) and, for a normal repo, inspects its remote — reusing the *exact* unexported `classifyDir` + `inspectRepo` logic `Walk` applies per directory. It returns exactly one of: `isRepo=true` with `Found` populated (normal repo with a usable remote); `isRepo=false` with `skipReason` set to a `Reason*` constant (worktree / bare repo / no-remote); or `isRepo=false` with `skipReason==""` (a plain non-git dir). `err` is non-nil only on a fatal `git` failure (wraps `proc.ErrNotFound` for `errors.Is` matching). The exported-seam choice is a Design Decision (below). The CLI canonicalizes `dir` (via the shared `validateConfigDir`) before calling, and `ClassifyOne` stays UI-free like the rest of the package.

## `tty.go` — TTY seam + no-TTY sentinel

Houses the package-level `isTTY` seam (swappable in tests via `withIsTTY` / a `TestMain` default of `true` in `testutil_test.go`, since the test binary runs with stdin redirected), the `errNoTTY` sentinel, and the `noTTYHint` stderr constant (1x1u). The seam mirrors `idea`'s `internal/idea.IsTTY` and hop's own package-level seam idiom (`pickResolve`, `pickOne`, `listWorktrees`). It **probes `/dev/tty`** (opens it and runs `term.IsTerminal` on the result) — **not `os.Stdin`** — see the Design Decision below. Consumed by `resolve.go::resolveByName` (before `pickResolve`) and `config_rm.go::pickRepo` (before `pickOne`); `errNoTTY` maps to exit 3 in `main.go::translateExit` and `shim_plan.go::shimResolveErr`. See [cli/match-resolution](/cli/match-resolution.md) and [cli/agent-non-interactive-usage](/cli/agent-non-interactive-usage.md).

## Design Decisions

### Subcommand list lives only in cobra

**Decision**: The shim hard-codes **zero** subcommand names. The binary's `isKnownSubcommand` (`shim_plan.go`) walks the live `newRootCmd().Commands()`, so the subcommand list has exactly one source of truth (cobra registration).

**Why**: Structural fix for the "stale shim" bug class — an already-open interactive shell cannot drift out of sync with the binary's subcommand set, because the shim branches only on the fixed 3-keyword protocol, never on subcommand names. Changes that add/remove/promote subcommands need no shim change.

*Introduced by*: gyo0

### posixInit is a single self-contained hop() — partial-capture immunity

**Decision**: `posixInit` emits **one** top-level shell function, `hop()`, with the PASSTHROUGH cd side-channel body (the `WT_CD_FILE` mktemp/`-s`/`cd`/`rm -f` dance) inlined directly into its `PASSTHROUGH)` case arm (`shell_init.go`). There is no standalone `_hop_passthrough()` sibling function.

**Why**: Defends against **partial capture** — a second facet of the "stale shim" bug class (alongside the name-drift fix above). Claude Code's shell-snapshot mechanism captures shell functions per-function; a dispatcher that depends on a separately-defined top-level sibling can be captured without it, making every PASSTHROUGH command (`where`, `ls`, `add`, `rm`, `clone`, `config`, `pull`/`push`/`sync`, `--help`, `--version`, …) die with `_hop_passthrough: command not found` even when the binary, config, and resolution logic are all correct. Inlining is the only form **provably immune** to partial capture: there is no second definition object for a snapshotter to drop, and it matches the file's "logic-free interpreter" framing. The inlined body keeps the `WT_CD_FILE` semantics verbatim, so the unified cd side-channel (`open`/`clone <url>` handoff) is unaffected; the `__complete*` arm, `CD`/`RUN_IN_PARENT` arms, defensive `*)` fallback, and the `h()` alias are ordinary arms of the same single function. See [cli/agent-non-interactive-usage](/cli/agent-non-interactive-usage.md) for how this ties into the broader agent contract.

**Rejected**: Nesting `_hop_passthrough` inside `hop()` — a nested function is still a separate definition object a snapshotter could in principle split.

*Introduced by*: 1x1u

### TTY probe targets /dev/tty, not os.Stdin

**Decision**: `isTTY` opens `/dev/tty` and runs `term.IsTerminal` on the result — it never keys on `os.Stdin`.

**Why**: fzf opens the controlling terminal directly for its picker UI and reads the candidate list hop pipes in from stdin separately (`internal/fzf.Pick` feeds a `strings.Reader` as fzf's stdin). An agent/CI run with no controlling terminal cannot open `/dev/tty`, so the probe correctly returns false there.

**Rejected**: Probing `os.Stdin.Fd()` — mis-reports `hop </dev/null` (or any stdin redirect) at a real terminal as "no TTY" even though fzf could still run via `/dev/tty`. (Caught in PR review — 1x1u.)

*Introduced by*: 1x1u

### ClassifyOne is the exported scan seam

**Decision**: `scan.ClassifyOne` is the single-dir entry point exported to the CLI; it wraps the unexported `classifyDir` + `inspectRepo` pair.

**Why**: It is the smallest exported surface that gives the CLI what it needs — classification plus remote inspection — while keeping the package UI-free.

**Rejected**: Exporting `classifyDir` raw — it would leak the private `dirClass` enum and skip the remote-inspection step (`inspectRepo`) the CLI requires.
