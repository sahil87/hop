...
|--------|-------------|------|
| cli | Command-line surface — selection-first `hop <selection> <action>` grammar, `--shim-plan` protocol shim, subcommands, ad-hoc URL clone, match resolution, plural selection, and the agent / non-interactive contract (`--json`, no-TTY exit 3, `HOP_WRAPPER`) | [subcommands](cli/subcommands.md), [match-resolution](cli/match-resolution.md), [agent-non-interactive-usage](cli/agent-non-interactive-usage.md) |
