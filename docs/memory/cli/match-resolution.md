...
**Both fzf seams carry the no-TTY guard (change `1x1u`)** — there is exactly one guard point per seam: `resolveByName` checks `isTTY()` immediately before `pickResolve(...)`, and `config_rm.go::pickRepo` checks `isTTY()` immediately before `pickOne(...)`. Each returns `errNoTTY` (→ exit 3) without spawning fzf. Tests inject `withIsTTY(t, false)` and assert the picker seam is **never** invoked (the fake `pickResolve`/`pickOne` fatals the test if called) and that the file is left unmodified on the `hop rm` path.
