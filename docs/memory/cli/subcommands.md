...
| `hop update` | `update.go` | none (`cobra.NoArgs`); `--skip-brew-update` bool flag (default false) | Self-update via Homebrew. Detects brew install via `os.Executable` + `EvalSymlinks` + `/Cellar/` substring; non-brew installs exit 0 with a manual-update hint. `--skip-brew-update` skips ONLY the internal `brew update --quiet` tap-metadata refresh — the `brew info` version check, the "already up to date" short-circuit, and `brew upgrade` all still run. Cross-toolkit contract: the flag name and semantics are shared with sibling tools, so do not rename or broaden. Logic lives in `internal/update`; subprocess calls go through `internal/proc` |
...
2. **Refresh brew index**: `brew update --quiet` with a 30-second timeout (via `proc.Run` + `context.WithTimeout`). Failure exits 1. **Skipped entirely when `--skip-brew-update` is set** — the flag gates ONLY this step; steps 3-5 (the `brew info` version check, the "already up to date" short-circuit, and `brew upgrade`) run unchanged. Default (flag absent) preserves the original behavior. The flag is a cross-toolkit contract shared with sibling tools; the name `--skip-brew-update` and its narrow "skip only the tap-metadata refresh" semantics MUST NOT be renamed or broadened.
