...
| `hop help-dump` (**hidden**) | `help_dump.go` | none (`cobra.NoArgs`) | **Hidden** cobra subcommand (`Hidden: true`) that prints the CLI help tree as a JSON document to stdout and exits 0. Build-time-only — CI invokes it to publish `help/hop.json` into `sahil87/shll.ai` (see [build/release-pipeline](../build/release-pipeline.md)). Being `Hidden`, it is absent from `hop --help` AND self-filters out of its own output (the walk's `shouldSkipChild` drops any `Hidden` child, including itself). Pure Go (`encoding/json` + cobra), no subprocess. See [help-dump JSON contract](#hop-help-dump--json-help-tree-contract) below |
...
Implementation in `help_dump.go` (change `jr5f`); the factory `newHelpDumpCmd()` is wired into `newRootCmd()`'s `AddCommand(...)` alongside the user-facing subcommands. It is a **hidden** command (`Hidden: true`, `Args: cobra.NoArgs`) so it ships in the released binary — CI can invoke it — without appearing in `hop --help`. It exists solely to feed the shll.ai landing site's "Command reference" UI: CI runs it on `v*` tags and PRs the output into `sahil87/shll.ai` as `help/hop.json` (see [build/release-pipeline](../build/release-pipeline.md)). This is hop's slice of a 7-tool rollout that all publish the same JSON shape (the reference is `wt`'s `help/wt.json`).
...
- `captured_at` is left as the empty string `""` by the producer — it does NOT call `time.Now()`, keeping the dump deterministic and unit-testable. CI injects a date-floored UTC value (`date -u +%Y-%m-%dT00:00:00Z`) via `jq`.
...
Local ergonomics: `just help-dump` delegates to `scripts/help-dump.sh` (Thin Justfile principle — see [build/local](../build/local.md)), which builds hop via `scripts/build.sh` and pretty-prints the dump via `jq` for inspection.
