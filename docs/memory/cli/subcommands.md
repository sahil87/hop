...
| `hop config add <dir>` | `config_add.go` | exactly 1 dir (`cobra.ExactArgs(1)`); no flags | The single-dir, **non-recursive** sibling of `config scan`: classifies just `<dir>` via the new `scan.ClassifyOne` and, for a normal repo with a remote, **writes by default** (no print mode — the user named a specific dir) into `hop.yaml` via the same `buildScanPlan` + `yamled.MergeScan` path scan uses (convention → `default`; else invented group keyed off the slugified parent-dir basename). Validates `<dir>` via the shared `validateConfigDir` (same `Clean → EvalSymlinks → Stat` idiom as scan), requires an existing `hop.yaml` (same `config init` pointer message). Forgiving by design — a plain (non-git) dir, a worktree/bare/no-remote candidate, and an already-registered URL are all **no-ops at exit 0** with a stderr message. Exit codes: **2** on bad dir (`hop config add: '<dir>' is not a directory.`); **1** on missing `hop.yaml` / load error / write failure / `git` missing (`errSilent`); **0** on success AND on every forgiving no-op. Status lines to stderr: `added: <url>` + `wrote: <path>` on a real add. See [config/scan](../config/scan.md) and [config/yaml-schema](../config/yaml-schema.md) |
| `hop config rm [--stale]` | `config_rm.go` | none (`cobra.NoArgs`); `--stale` bool flag (default false) | **Single-select interactive** removal via fzf: loads repos (`repos.FromConfig`), builds picker lines via the shared `buildPickerLines`, pipes them through `fzf.Pick` (seam `var pickOne = fzf.Pick`), maps the selection back to its `Repo` by the unique **path** column (the resolve.go approach), and removes that repo's URL from its group via `yamled.RemoveURL`. With `--stale`, pre-filters candidates to repos whose resolved `Path` is missing from disk (`os.Stat`); zero stale → friendly message, exit 0, no picker. Removing a group's last URL leaves an empty-group placeholder (see [config/yaml-schema](../config/yaml-schema.md)). Exit codes: **1** on fzf-missing (`fzfMissingHint` + `errSilent`), missing `hop.yaml`, load error, or a real `RemoveURL` write failure; **130** on fzf cancel (Esc/Ctrl-C, via `errFzfCancelled`); **0** on success, nothing-to-remove, nothing-stale, AND the forgiving `RemoveURL` not-found case. Status line to stderr: `removed: <url>` + `wrote: <path>` |
...
- `buildPickerLines(rs) []string` — builds the tab-separated lines piped to fzf. When two repos share a `Name`, the displayed first column gets a `[<group>]` suffix. The path column (used for match-back) is unique per repo. Reused by `hop config rm`'s `pickRepo` (config_rm.go), which pipes the lines through the `pickOne` seam and maps the selection back to its `Repo` by the same unique path column.
- `validateConfigDir(userArg, cmdName, stderr) (canonical string, ok bool)` — shared dir-argument validator in `config_scan.go` (extracted in the `config add` rework, A-018). Applies `filepath.Clean → filepath.EvalSymlinks → os.Stat (is-dir)`; on any failure emits `<cmdName>: '<userArg>' is not a directory.` and returns `ok=false` so the caller exits 2. Called by both `config scan` and `config add` (`cmdName` is `"hop config scan"` / `"hop config add"`), replacing the previously-duplicated `validateScanDir`/`validateAddDir`.
- `pickRepo(rs) (*repos.Repo, error)` + the package-level `var pickOne = fzf.Pick` seam (both in `config_rm.go`, change `260602-n1me-config-add-rm-folders`) — `pickRepo` builds the picker lines, runs `pickOne(ctx, lines, "")` (single-select), then maps the selected line back to its `Repo` by splitting on tab and matching the unique path column (`parts[1]`), the same approach `resolveByName` uses. The `pickOne` seam mirrors the `listWorktrees` / `runInteractive` seam idiom: tests inject a fake selection to exercise the map-back + `RemoveURL` integration without spawning a real `fzf` (`resolve.go` calls `fzf.Pick` directly with no cmd-level seam, so its fzf path is unit-untestable — `config rm` deliberately added the seam to be testable). `pickRepo` maps `proc.ErrNotFound` → `errFzfMissing` and fzf exit-130 → `errFzfCancelled`.
...
|---|---|---|
| `fzf` | `where.go::fzfMissingHint` | `hop: fzf is not installed. Install it: brew install fzf (macOS) or apt install fzf (Debian).` (reused by `hop config rm` — its `pickRepo` maps `proc.ErrNotFound` from `fzf.Pick` to `errFzfMissing`, the CLI then writes this hint and returns `errSilent`) |
| `git` | `clone.go::gitMissingHint` | `hop: git is not installed.` (also reused by `hop config scan` — lazy-checked at the first `git remote` invocation; empty scan trees with zero `.git` discoveries succeed without invoking `git`. Also reused by `hop config add` — lazy-checked when its single-dir `scan.ClassifyOne` inspects a `.git` candidate's remote. Also reused by `hop pull`, `hop push`, and `hop sync` — emitted once and aborts the batch immediately on the first `proc.ErrNotFound`) |
...
**Node fields** come from structured cobra accessors (NOT regex-parsing `-h` output): `name = cmd.Name()`, `path = cmd.CommandPath()`, `short = cmd.Short`, `usage = cmd.UseLine()`. The `text` field is built **uniformly** by `nodeText`: `cmd.Long + "\n\n" + cmd.UsageString()` when `cmd.Long != ""`, else `cmd.UsageString()` alone — so the root node's `text` carries the full `rootLong` narrative followed by the usage block, and the `-R` / tool-form invocations (documented in `rootLong` but living outside the cobra tree in `extractDashR`) are *described* in the dump even though they are not separate nodes.

**Child filtering** (`shouldSkipChild`) drops, during the recursive walk: any child named `"completion"` or `"help"` (cobra auto-generated), any child with `Hidden == true` (which drops `help-dump` itself — the self-filter), and any `IsAdditionalHelpTopicCommand()` (defensive). So `root.commands` contains the eight user-facing subcommands (`clone`, `pull`, `push`, `sync`, `ls`, `shell-init`, `config`, `update`) and excludes `completion`, `help`, and `help-dump`.
